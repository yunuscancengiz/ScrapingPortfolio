...
class ColdwellBanker:
    ...
    def get_office_urls(self):
        for page_url in self.page_urls:
            r = httpx.get(page_url, headers=self.headers)
            soup = BeautifulSoup(r.content, 'lxml')

            office_url_list = soup.find_all('h3', attrs={'class':'title'})
            for office_url in office_url_list:
                self.office_urls.append('https://www.cb.com.tr' + office_url.find('a').get('href'))

    
    def get_broker_info(self):
        counter = 1
        for office_url in self.office_urls:
            r = httpx.get(office_url, headers=self.headers)
            soup = BeautifulSoup(r.content, 'lxml')
            ...
            for broker in brokers:
                broker_urls.append('https://www.cb.com.tr' + broker.get('href'))

            for broker_url in broker_urls:
                r = httpx.get(broker_url, headers=self.headers)
                soup = BeautifulSoup(r.content, 'lxml')
