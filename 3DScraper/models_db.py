...
class Model:
    def __init__(self, url, model_code, name_en, name_tr, description_en, description_tr, template_description_en, template_description_tr, total_price, volume, status, stl_size):
        self.url = url
...
class ModelDB:
    ...
    def add_model(self, model: Model):
        query = f'''
        INSERT INTO {self.table} (Link, Code, NameEN, NameTR, DescriptionEN, DescriptionTR, TemplateDescriptionEN, TemplateDescriptionTR, TotalPrice, Volume, Status, Size1)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        '''
        try:
            self.cursor.execute(query, (
                model.url, model.model_code, model.name_en, model.name_tr,
    ...
    def show_all_models(self):
        ...
        if not models:
            ...
            for model_data in models:
                model = Model(
                    url=model_data[1], model_code=model_data[2], name_en=model_data[3],
                    name_tr=model_data[4], description_en=model_data[5],
                    description_tr=model_data[6], template_description_en=model_data[7], template_description_tr=model_data[8], total_price=model_data[9],
                    volume=model_data[10], status=model_data[11], stl_size=model_data[12]
                )
                print(model)
    ...
    def update_model(self, model_code: str, model: Model):
        query = f'''
        UPDATE {self.table}
        SET Link = ?, NameEN = ?, NameTR = ?, DescriptionEN = ?, DescriptionTR = ?, TemplateDescriptionEN = ?, TemplateDescriptionTR = ?, TotalPrice = ?, Volume = ?, Status = ?, Size1 = ?
        WHERE Code = ?
        '''
        self.cursor.execute(query, (
            model.url, model.name_en, model.name_tr, model.description_en,
    ...
    def update_column(self, model_code: str, column: str, new_value):
        valid_columns = {'Link', 'Code', 'NameEN', 'NameTR', 'DescriptionEN', 'DescriptionTR', 'TemplateDescriptionEN', 'TemplateDescriptionTR', 'TotalPrice', 'Volume', 'Status', 'Size1'}
    ...
    def search_and_export(self, keyword: str, export_path: str = None):
        ...
        WHERE Code LIKE ? OR NameEN LIKE ? OR NameTR LIKE ? OR DescriptionEN LIKE ? OR DescriptionTR LIKE ? OR TemplateDescriptionEN LIKE ? OR TemplateDescriptionTR LIKE ? OR Status LIKE ?
        '''
        keyword = f"%{keyword}%"
        self.cursor.execute(query, (keyword, keyword, keyword, keyword, keyword, keyword, keyword, keyword))
        ...
        columns = ['ID', 'Link', 'Code', 'NameEN', 'NameTR', 'DescriptionEN', 'DescriptionTR', 'TemplateDescriptionEN', 'TemplateDescriptionTR', 'TotalPrice', 'Volume', 'Status', 'Size1']
...
class ExcelToDBTool:
    ...
    def process_excel(self, excel_path: str):
        ...
        required_columns = ['Link', 'Code', 'NameEN', 'NameTR', 'DescriptionEN', 'DescriptionTR', 'TemplateDescriptionEN', 'TemplateDescriptionTR', 'TotalPrice', 'Volume', 'Status', 'Size1']
